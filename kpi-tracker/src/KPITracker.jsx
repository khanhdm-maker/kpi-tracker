...
function SummaryCard({ label, value, sub, pctVal, color }) {
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: "16px 20px", boxShadow: "0 1px 3px #0001" }}>
      <div style={{ fontSize: 11, letterSpacing: 2, color: C.muted, marginBottom: 6, fontWeight: 600 }}>{label}</div>
      ...
      {pctVal !== null && (
        <>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: st.color }}>{pctVal}% of KPI</span>
...
function ResultCard({ label, actual, target, pctVal, color }) {
  ...
  return (
    <div style={{ background: C.card, border: `1.5px solid ${actual !== null ? color + "55" : C.border}`, borderRadius: 12, padding: 18, flex: 1, boxShadow: actual !== null ? `0 2px 12px ${color}15` : "none" }}>
      <div style={{ fontSize: 11, letterSpacing: 3, color: C.muted, marginBottom: 6, fontWeight: 600 }}>{label}</div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: 17, fontWeight: 900, color: pctVal !== null ? st.color : "#cbd5e1" }}>{pctVal !== null ? `${pctVal}%` : "—"}</span>
...
export default function KPITracker() {
  ...
  const derived = monthlyActuals.map((a, i) => {
    ...
    let rev = parseMoney(a.revenue);
    let cost = parseMoney(a.cost);
    if (hasDailyRev) rev = dailySumRev;
    if (hasDailyCost) cost = dailySumCost;
  ...
  const setDailyField = (mi, day, field, val) =>
    setDailyActuals((prev) =>
      prev.map((month, i) =>
        i !== mi ? month : month.map((dayData, di) => di === day - 1 ? { ...dayData, [field]: val } : dayData)
      )
    );

  const monthlyChartData = KPI.map((k, i) => ({
    name: MONTHS_SHORT[i],
    "KPI Target": k[chartMetric === "profit" ? "profit" : "revenue"],
  ...
  const TAB = (key, label) => (
    <button key={key} onClick={() => setView(key)} style={{
      padding: "9px 16px",
      background: view === key ? "#1e40af" : "transparent",
      color: view === key ? "#fff" : C.muted,
      border: "none", cursor: "pointer",
      fontFamily: "inherit", fontSize: 13, fontWeight: 700,
      letterSpacing: 1, borderRadius: 8, transition: "all .15s",
    }}>{label}</button>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'DM Mono','Courier New',monospace", color: C.text }}>
      ...
      <div style={{ background: "#1e3a8a", padding: "18px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div>
          <div style={{ fontSize: 11, letterSpacing: 4, color: "#93c5fd", marginBottom: 3 }}>KPI PHẢI ĐẠT 2026 · $33M REV · $3M PROFIT · 10% ROI</div>
      ...
      {/* SUMMARY STRIP */}
      <div style={{ padding: "20px 28px 0", display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(170px,1fr))", gap: 14 }}>
        ...
        <SummaryCard label="MONTHS FILED" value={`${filledCount} / 12`} sub="months with data" pctVal={filledCount > 0 ? +((filledCount / 12) * 100).toFixed(0) : null} />
      </div>
      ...
      {view === "month" && (
        <div style={{ padding: "20px 28px 32px", display: "flex", flexDirection: "column", gap: 18 }}>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {KPI.map((k, i) => {
              const d2 = derived[i];
              const filled = d2.rev !== null;
              const p = pct(d2.rev, k.revenue);
              const st = statusOf(p);
              return (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 24, boxShadow: "0 2px 8px #0001", display: "flex", flexDirection: "column", gap: 18 }}>
              ...
              {[
                { field: "cost", label: "ACTUAL COST", placeholder: "e.g. 1800000", hint: "Total operating cost · overridden by daily entries", color: C.cost },
                { field: "revenue", label: "ACTUAL REVENUE", placeholder: "e.g. 2100000", hint: "Total sales revenue · overridden by daily entries", color: C.rev },
                { field: "profit", label: "ACTUAL PROFIT", placeholder: "auto-calculated", hint: "Leave blank → auto = Revenue − Cost", color: C.profit },
              ].map(({ field, label, placeholder, hint, color }) => (
                <div key={field}>
                  <label style={{ fontSize: 11, letterSpacing: 3, color, display: "block", marginBottom: 6, fontWeight: 700 }}>{label}</label>
                  <input
                    type="text" value={a[field]}
                  ...
                  />
                  <div style={{ fontSize: 12, color: C.muted, marginTop: 4 }}>{hint}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                ...
                <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16 }}>
                  ...
                  <div style={{ fontSize: 20, fontWeight: 900, color: d.roi !== null ? statusOf(pct(d.roi, ROI_KPI)).color : "#cbd5e1" }}>{d.roi !== null ? `${d.roi.toFixed(2)}%` : "—"}</div>
                  <div style={{ fontSize: 13, color: C.muted }}>KPI: {ROI_KPI}%</div>
                  {d.roi !== null && <div style={{ marginTop: 8, height: 4, background: C.light, borderRadius: 4, overflow: "hidden" }}><div style={{ height: "100%", width: `${Math.min((d.roi / ROI_KPI) * 100, 100)}%`, background: statusOf(pct(d.roi, ROI_KPI)).color, borderRadius: 4, transition: "width .6s" }} /></div>}
                </div>
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                {[["← PREV", () => setActiveMonth(Math.max(0, m - 1)), m === 0], ["NEXT →", () => setActiveMonth(Math.min(11, m + 1)), m === 11]].map(([label, fn, dis]) => (
                  <button key={label} onClick={fn} disabled={dis} style={{ flex: 1, padding: "10px 0", borderRadius: 8, border: `1px solid ${dis ? C.border : "#1e40af"}`, background: dis ? C.light : "#eff6ff", color: dis ? "#cbd5e1" : "#1e40af", cursor: dis ? "not-allowed" : "pointer", fontFamily: "inherit", fontSize: 14, fontWeight: 700 }}>{label}</button>
                ))}
              </div>
            </div>
          </div>
        ...
        <div style={{ padding: "20px 28px 32px", display: "flex", flexDirection: "column", gap: 18 }}>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {KPI.map((k, i) => {
              ...
              return (
                <button key={i} onClick={() => { setActiveMonth(i); setActiveDay(1); }} style={{ padding: "6px 14px", borderRadius: 8, fontFamily: "inherit", fontSize: 13, fontWeight: activeMonth === i ? 800 : 500, border: activeMonth === i ? "2px solid #1e40af" : `1px solid ${C.border}`, background: activeMonth === i ? "#eff6ff" : C.card, color: activeMonth === i ? "#1e40af" : C.muted, cursor: "pointer", position: "relative" }}>
                  {k.label.slice(0, 3).toUpperCase()}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: 18 }}>
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden" }}>
              <div style={{ padding: "12px 16px", borderBottom: `1px solid ${C.border}`, fontSize: 12, letterSpacing: 2, color: C.muted, fontWeight: 700 }}>{KPI[m].label.toUpperCase()} · DAYS</div>
                ...
                {Array.from({ length: DAYS_IN_MONTH[m] }, (_, i) => {
                  ...
                  return (
                    <div key={day} onClick={() => setActiveDay(day)} style={{ padding: "10px 16px", background: activeDay === day ? "#eff6ff" : "transparent", borderLeft: activeDay === day ? "3px solid #1e40af" : "3px solid transparent", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      {filled && <span style={{ fontSize: 14, color: "#f97316" }}>✓</span>}
                    </div>
                  );
                })}
              </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 24 }}>
                <div style={{ marginBottom: 18 }}>
                  ...
                  <div style={{ fontSize: 22, fontWeight: 900, color: C.text }}>{KPI[m].label} — Day {activeDay}</div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
                  {[{ field: "cost", label: "DAILY COST", color: C.cost }, { field: "revenue", label: "DAILY REVENUE", color: C.rev }].map(({ field, label, color }) => {
                    const val = dailyActuals[m][activeDay - 1][field];
                    return (
                      <div key={field}>
                        <label style={{ fontSize: 11, letterSpacing: 3, color, display: "block", marginBottom: 6, fontWeight: 700 }}>{label}</label>
                        <input type="text" value={val} onChange={(e) => setDailyField(m, activeDay, field, e.target.value)} placeholder="0" style={inputStyle(!!val)}
                          onFocus={(e) => { e.target.style.border = `1.5px solid ${color}`; }}
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 18 }}>
                  {[["← PREV DAY", () => setActiveDay(Math.max(1, activeDay - 1)), activeDay === 1], ["NEXT DAY →", () => setActiveDay(Math.min(DAYS_IN_MONTH[m], activeDay + 1)), activeDay === DAYS_IN_MONTH[m]]].map(([label, fn, dis]) => (
                    <button key={label} onClick={fn} disabled={dis} style={{ flex: 1, padding: "9px 0", borderRadius: 8, border: `1px solid ${dis ? C.border : "#1e40af"}`, background: dis ? C.light : "#eff6ff", color: dis ? "#cbd5e1" : "#1e40af", cursor: dis ? "not-allowed" : "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 700 }}>{label}</button>
                  ))}
                </div>
              </div>
              <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 20 }}>
                <div style={{ fontSize: 11, letterSpacing: 2, color: C.muted, fontWeight: 700, marginBottom: 4 }}>{KPI[m].label.toUpperCase()} · CUMULATIVE vs KPI</div>
                {[
                  ...
                  return (
                    <div key={label} style={{ marginBottom: 16 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                        <span style={{ fontSize: 11, letterSpacing: 2, color: C.muted, fontWeight: 700 }}>{label} <strong style={{ color: actual !== null ? color : "#cbd5e1" }}>{fmtBig(actual)}</strong></span>
                        {p !== null && <span style={{ fontSize: 13, fontWeight: 800, color: st.color }}>{p}%</span>}
                      </div>
                      {target && <div style={{ height: 8, background: C.light, borderRadius: 8, overflow: "hidden", marginBottom: 6 }}><div style={{ height: "100%", borderRadius: 8, width: p !== null ? `${Math.min(p, 100)}%` : "0%", background: p !== null ? st.color : C.light, transition: "width .6s" }} /></div>}
                      {dailyNeeded !== null && dailyNeeded > 0 && <div style={{ fontSize: 12, color: C.muted }}>Need <strong style={{ color }}>{fmtBig(dailyNeeded)}/day</strong> for {daysLeft} days left</div>}
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              ...
              <tbody>
                {KPI.map((k, i) => {
                  const d2 = derived[i];
                  const rp = pct(d2.rev, k.revenue);
                  const pp = pct(d2.prof, k.profit);
                  ...
                  return (
                    <tr key={i} onClick={() => { setActiveMonth(i); setView("month"); }}
                      ...
                      <td style={{ padding: "11px 14px" }}>
                        {rp !== null ? <span style={{ color: rs.color, fontWeight: 700 }}>{rp}%</span> : <span style={{ color: "#cbd5e1" }}>—</span>}
                      </td>
              ...
              <tfoot>
                <tr style={{ background: "#e0f2fe", borderTop: "2px solid #bae6fd" }}>
                  <td style={{ padding: "12px 14px", fontWeight: 900, color: "#0369a1" }}>TOTAL</td>
        ...
        <div style={{ padding: "20px 28px 32px", display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
            <div style={{ display: "flex", gap: 4, background: C.light, borderRadius: 8, padding: 4 }}>
              {[["monthly", "12-Month View"], ["daily", "Daily View"]].map(([t, label]) => (
                <button key={t} onClick={() => setChartType(t)} style={{ padding: "6px 14px", borderRadius: 6, background: chartType === t ? "#fff" : "transparent", color: chartType === t ? "#1e40af" : C.muted, border: chartType === t ? `1px solid ${C.border}` : "1px solid transparent", cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 700 }}>{label}</button>
              ))}
            </div>
            {chartType === "monthly" && (
              <div style={{ display: "flex", gap: 4, background: C.light, borderRadius: 8, padding: 4 }}>
            ...
            {chartType === "daily" && (
              <div style={{ display: "flex", gap: 4, background: C.light, borderRadius: 8, padding: 4 }}>
                {KPI.map((k, i) => (
                  <button key={i} onClick={() => setActiveMonth(i)} style={{ padding: "5px 10px", borderRadius: 6, background: activeMonth === i ? "#fff" : "transparent", color: activeMonth === i ? "#1e40af" : C.muted, border: activeMonth === i ? `1px solid ${C.border}` : "1px solid transparent", cursor: "pointer", fontFamily: "inherit", fontSize: 12, fontWeight: 700 }}>{k.label.slice(0, 3)}</button>
                ))}
              </div>
            )}
          </div>
          ...
          {chartType === "monthly" && (
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "24px 20px" }}>
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: 2, color: C.muted, marginBottom: 20 }}>{chartMetric.toUpperCase()} — ACTUAL vs KPI TARGET</div>
          ...
          {chartType === "daily" && (
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "24px 20px" }}>
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: 2, color: C.muted, marginBottom: 20 }}>DAILY BREAKDOWN — {KPI[m].label.toUpperCase()}</div>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={dailyChartData} margin={{ top: 8, right: 20, left: 10, bottom: 8 }} barGap={2} barSize={8}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} />
                  <XAxis dataKey="name" tick={{ fontSize: 11, fill: C.muted, fontFamily: "inherit" }} axisLine={false} tickLine={false} interval={2} />
                  <YAxis tickFormatter={fmtShort} tick={{ fontSize: 11, fill: C.muted, fontFamily: "inherit" }} axisLine={false} tickLine={false} width={60} />
                  <Tooltip content={<CustomTooltip />} />
                  <Legend wrapperStyle={{ fontSize: 13, fontFamily: "inherit" }} />
          ...
          {chartType === "monthly" && (
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "24px 20px" }}>
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: 2, color: C.muted, marginBottom: 20 }}>REVENUE · COST · PROFIT — ALL MONTHS</div>
